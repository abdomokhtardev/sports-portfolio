/**
 * SocialIcon — أيقونات معروفة + fallback مرن لأي منصة
 * @param {{ platform?: string, label?: string, className?: string }} props
 */
...
const LinkFallback = ({ label, platform, className }) => (
  <span className={`inline-flex items-center justify-center w-5 h-5 text-[10px] font-bold uppercase ${className}`}>
    {(label || platform || '↗').slice(0, 2)}
  </span>
);

const SocialIcon = ({ platform = '', label = '', className = '' }) => {
  const key = platform.toLowerCase().replace(/[^a-z0-9]/g, '');
  const icon = ICONS[key];
  if (icon) return <span className={className}>{icon}</span>;
  return <LinkFallback label={label} platform={platform} className={className} />;
};
